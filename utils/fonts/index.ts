...
export const FONT_CATEGORIES = {
  ...
  serif: { label: "Serif", fallback: "ui-serif, Georgia, serif" },
  monospace: {
    label: "Monospace",
    fallback:
      "ui-monospace, 'Cascadia Code', 'Source Code Pro', Menlo, Monaco, Consolas, 'Courier New', monospace",
  },
  display: { label: "Display", fallback: "ui-serif, Georgia, serif" },
  handwriting: { label: "Handwriting", fallback: "cursive" },
} as const;
...
export function extractFontFamily(fontFamilyValue: string): string | null {
  if (!fontFamilyValue) return null; // Split by comma and get the first font
...
export function isFontLoaded(family: string, weight = "400"): boolean {
  if (typeof document === "undefined" || !document.fonts) return false; // Use the native FontFaceSet.check() method
