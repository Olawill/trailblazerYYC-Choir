...
import { editMusic, newMusic } from "@/actions/playlist";
...
export const EditMusicForm = ({
  ...
  musicId: string;
  setEditTrackOpen: Dispatch<SetStateAction<boolean>>;
}) => {
  ...
  const { data: musicInfo, isLoading } = useQuery({
    queryKey: ["editMusic", musicId],
    queryFn: () => getMusicInfo(musicId),
  });
  ...
  useEffect(() => {
    if (musicInfo) {
      form.reset({
        ...
        link: musicInfo.link || undefined,
        playlistIds: musicInfo.playlists.map((p) => p.id) || [],
        authorIds: musicInfo.authors.map((a) => a.id) || [],
        content: musicInfo.contents || [],
      });

      setListName(musicInfo.playlists.map((p) => p.name));
      setAuthorNames(musicInfo.authors.map((a) => a.name));
    }
  }, [musicInfo, form]);
  ...
  const onSubmit = (values: z.infer<typeof NewMusicSchema>) => {
    ...
    startTransition(async () => {
      const data = await editMusic(musicId, values);
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      disabled={isPending}
                      {...field}
                      // defaultValue={musicInfo?.link || undefined}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <MusicContent
                      reload={formSubmitted}
                      contents={musicInfo?.contents}
                    />
                  </FormControl>
                  <FormMessage />
                </FormItem>
